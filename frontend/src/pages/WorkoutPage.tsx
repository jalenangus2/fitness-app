...
import { Plus, Zap, CheckCircle, Trash2, ChevronDown, Play, Timer, X, Minus, BarChart2, Dumbbell } from 'lucide-react'
import {
        ...
        <div className="space-y-4">
          ...
          {sessions.length === 0 && (
            <Card className="text-center py-10">
              ...
              <p className="text-sm text-slate-500 mt-1">Start a plan to begin tracking.</p>
            </Card>
          )}
          ...
          {createMode === 'manual' && (
            <div className="space-y-3 pt-4 border-t border-slate-700">
              <h4 className="text-sm text-slate-300 font-medium flex justify-between">
                Exercises
                <button onClick={() => setManualExercises([...manualExercises, { name: '', sets: 3, reps: '10' }])} className="text-indigo-400 text-xs flex items-center"><Plus size={12} /> Add</button>
              </h4>
                ...
                {logs.map((log, i) => (
                  <div key={log.id} className="flex justify-between text-xs text-slate-400 bg-slate-800/50 px-3 py-1.5 rounded">
                    ...
                    <span className="text-slate-300">
                      {log.weight_lbs ? `${log.weight_lbs} lbs` : '—'} × {log.reps ?? '—'} reps
                    </span>
